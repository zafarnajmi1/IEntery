...
//        txtpassword.leadingViewMode = .always
        ...
        if base64String != nil {
            let decodedData = NSData(base64Encoded: base64String, options: [])
                if let data = decodedData {
                    ...
                    self.bas64userimg = decodedimage ?? UIImage()
                } else {
    ...
    func saveBase64StringToPDF(_ base64String: String,optionName:String) {
        
        
        
        let documentsUrl =  FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first!
        let fileURL = documentsUrl.appendingPathComponent("\(optionName).pdf") //.appendingPathExtension(".pdf")
        do {
            ...
            print(error,"helooooooooooooooooo not Write")
        }
...
//        let dateVar = Date(timeIntervalSinceNow: TimeInterval(milisecond)!/1000)
